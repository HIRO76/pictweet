...
describe User do
  describe '#create' do
    ...
    it "is invalid with a duplicate email address" do
      ...
      another_user.valid?
      expect(usanother_userer.errors[:email]).to include("has already been taken")
    end

    it "is invalid with a nickname that has more than 7 characters" do
      user = build(:user, nuckname: "aaaaaaaa")
      user.valid?
      expect(user.errors[:nuckname][0]).to include("is too long")
    end
